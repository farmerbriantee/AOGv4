 ...
 if (PWM_Frequency == 1) 
  ...
  { 
   ...
   if (ether.begin(sizeof Ethernet::buffer, mymac, CS_Pin) == 0)
    ...
    //set up the pgn for returning udpData for autosteer
    toSend[0] = 0x7F;
    toSend[1] = 0xFD;
...
void udpSteerRecv(uint16_t dest_port, uint8_t src_ip[IP_LEN], uint16_t src_port, byte *udpData, uint16_t len)
{
  ...
  if (udpData[0] == 0x7F && udpData[1] == 0xFE) //Data
  {
    ...
    distanceFromLine = (float)(udpData[4] << 8 | udpData[5]);   //high,low bytes     
  ...
  if (udpData[0] == 0x7F && udpData[1] == 0xFA) //Machine Data
  {
    relayHi = udpData[2];  
    relay = udpData[3];  
    gpsSpeed = udpData[4] * 0.25;  //actual speed times 4, single byte
    uTurn = udpData[5];
  }

 if (udpData[0] == 0x7F && udpData[1] == 0xFB) //Arduino Settings
  {
    //set0
  ...
  //autosteer settings
  if (udpData[0] == 0x7F && udpData[1] == 0xFC)
  {
    ...
    //for (int i = 0; i < len; i++) {Serial.print(udpData[i],HEX); Serial.print("\t"); } Serial.println("<--"); }
    highLowPerDeg = (steerSettings.highPWM - steerSettings.lowPWM) / LOW_HIGH_DEGREES;
  }
}
